...
- 右クリックで**Windows標準のシェルコンテキストメニュー**を表示。一覧の空白部分ではフォルダー自体のメニュー（新規作成・ターミナルで開く・プロパティ等）、Shiftを押しながらで拡張項目（PowerShellをここで開く等）も表示
- 一覧で文字を入力すると、その文字で始まる項目へジャンプ
- **ドラッグ&ドロップ**: ファイル一覧から外部アプリ（Explorer・ブラウザ等）へのドラッグに加え、ファイル一覧・ツリーへのドロップにも対応。フォルダー行/ツリーのノードに重ねるとそのフォルダーへ、一覧の空白部分なら表示中のフォルダーへ。移動/コピーの判定（同じドライブなら移動、Shift/Ctrl/Alt修飾、右ドラッグのメニュー）や zip・exe へのドロップ、ゴミ箱へのドロップはエクスプローラーと同じ（シェル自身のドロップ処理に委譲）。ドラッグ中は一覧の上下端で自動スクロール、ツリーのノードに少し留まると展開
